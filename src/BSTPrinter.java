...
public class BSTPrinter {
    ...
    public static <T extends Comparable<?>> void printNode(SymbolTableNode root) {
        int maxLevel = BSTPrinter.maxLevel(root);
        printNodeInternal(Collections.singletonList(root), 1, maxLevel);
    }

    private static <T extends Comparable<?>> void printNodeInternal(List<SymbolTableNode> nodes, int level, int maxLevel) {
        ...
        List<SymbolTableNode> newNodes = new ArrayList<SymbolTableNode>();
        for (SymbolTableNode node : nodes) {
            if (node != null) {
    ...
    private static <T extends Comparable<?>> int maxLevel(SymbolTableNode node) {
        if (node == null)
            return 0;
